...
function handleChangeVisibility(elBalance, elBtn) {
  ...
  if (currentIsShown !== 'true') {
    // Bikin bintang sesuai jumlah digit balanceValue (tanpa "Rp. ")
    const angka = balanceValue.replace(/\D/g, ''); // ambil hanya angka
    const hidden = '*'.repeat(angka.length);

    elBalance.innerHTML = 'Rp. ' + hidden;
